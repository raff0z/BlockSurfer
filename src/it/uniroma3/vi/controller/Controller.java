...
public class Controller extends HttpServlet {
	...
	String actionName = this.comand2action.get(command); // Preso
							     // dinamicamente il
							     // tipo di comando,
							     // prelevo tramite
							     // il mapping
	...
	if (actionName == null) {
	    ...
	    // Se non è null provo a creare dinamicamente il tipo di azione
	    // associata
	    Action action = null;

	    try {
		// La creazione di azione necessita del parametro
		// HettpServletRequest, lo imposto prelevando il costruttore
		// dalla classe azione e
	...
	// il metodo split separa la stringa in n stringhe (che riporta in un
	// array) secondo una espressione regolare
	String[] path = servletPath.split("/"); // separo l'url cosi da poter
						// prendere l'ultima stringa
						// dopo /

	String command = path[path.length - 1].split("\\.")[0]; // prendo
								// l'ultima
								// stringa del
								// percorso e la
								// separo con il
